...
def capture_and_highlight():
    ...
    cursor_pos = (x, y) # เก็บตำแหน่งเมาส์ไว้ด้วย
...
def capture_and_queue_for_ocr():
    """
    [ฟังก์ชันใหม่] แก้ปัญหา App Not Responding โดยย้าย OCR ไปทำใน Worker Thread
    ฟังก์ชันนี้จะทำแค่จับภาพแล้วส่งไปที่ Queue ทันที
    """
    capture_width = 400
    capture_height = 300
    x, y = pyautogui.position()
    ...
    translation_queue.put({'screenshot': screenshot, 'cursor_pos': (x, y), 'region_top_left': (left, top)})
...
def translation_worker():
    ...
    """
    while True:
        # [แก้ไข] รับข้อมูลภาพมาจาก Queue
        job = translation_queue.get()
        if job is None:
            break
        ...
        try:
            # 1. ทำ OCR ใน Worker Thread นี้
            data = pytesseract.image_to_data(screenshot, lang='eng', output_type=pytesseract.Output.DICT)
        except pytesseract.pytesseract.TesseractError as e:
            print(f"Tesseract Error: {e}")
            emitter.show_tooltip.emit("<i>Tesseract Error</i>")
            continue # ไปรอรับงานต่อไป

        # 2. สร้าง Bounding Box ของคำต่างๆ
        text_boxes = []
        n_boxes = len(data['text'])
        for i in range(n_boxes):
            if data['text'][i].strip() != '':
        ...
        # 4. ถ้าเจอคำ ให้เริ่มแสดงผลและส่งไปแปล
        if found_box:
            emitter.show_tooltip.emit("<i>Loading...</i>")
            blink_highlight(found_box)
            translate_and_show(found_box) # เรียกฟังก์ชันแปลต่อไป
        else:
            cancel_highlight()

def translate_and_show(box): # ฟังก์ชันนี้เหมือนเดิม แต่ถูกเรียกจาก worker
    ...
    try:
        ...
        if hasattr(key, 'char') and key.char == '\x04':
            if 'ctrl' in current_keys:
                ...
                capture_and_queue_for_ocr() # [แก้ไข] เรียกใช้ฟังก์ชันใหม่ที่เร็วกว่า
                return
